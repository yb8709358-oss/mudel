...
import { SERVICE_CACHE_TAG } from '@/lib/api';
...
const SERVICE_ID = '4b6927c8-33ce-47d3-8518-88edd1bb7b92';
...
describe('admin service writes invalidate the public service cache', () => {
  it('revalidates the service tag after a successful create (POST)', async () => {
    ...
    expect(revalidateTag).toHaveBeenCalledWith(SERVICE_CACHE_TAG);
  });

  it('revalidates the service tag after a successful update (PATCH)', async () => {
    adminIdRoute.mockResolvedValue({ ok: true, status: 200 });
    const res = await PATCH(mockRequest('PATCH'), paramsFor(SERVICE_ID));
    expect(res).toEqual({ ok: true, status: 200 });
    expect(revalidateTag).toHaveBeenCalledTimes(1);
    expect(revalidateTag).toHaveBeenCalledWith(SERVICE_CACHE_TAG);
  });

  it('revalidates the service tag after a successful delete (DELETE)', async () => {
    adminIdRoute.mockResolvedValue({ ok: true, status: 200 });
    const res = await DELETE(mockRequest('DELETE'), paramsFor(SERVICE_ID));
    expect(res).toEqual({ ok: true, status: 200 });
    expect(revalidateTag).toHaveBeenCalledTimes(1);
    expect(revalidateTag).toHaveBeenCalledWith(SERVICE_CACHE_TAG);
  });
  ...
  it('does not revalidate on read-only GET list/detail requests', async () => {
    ...
    await idGET(mockRequest('GET'), paramsFor(SERVICE_ID));
    expect(revalidateTag).not.toHaveBeenCalled();
  });
});
